...
export default {
    ...
    aliases: ['gaajudl'],
    category: 'download',
    description: 'Download from kazi with real-time progress',
    usage: '.kazi <kazi-url>',
    async handler(sock, message, args, context) {
        const chatId = context.chatId || message.key.remoteJid;
        const text = args.join(' ').trim();
        if (!text) {
            return sock.sendMessage(chatId, { text: `*Usage:* .gaaju https://gaaju.nz/file/xxxx#xxxx` }, { quoted: message });
        }
        try {
