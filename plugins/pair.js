...
export default {
    ...
    category: 'general',
    description: 'Get session id for Kazi-md-bot-4X',
    ...
    async handler(sock, message, args, context) {
        ...
        try {
            const response = await axios.get(`https://Kazi-md-bot-4X-pair4.onrender.com/pair?number=${number}`, {
                timeout: 60000
            });
            if (response.data && response.data.code) {
                ...
                const successText = `✅ *Kazi-md-bot-4X PAIRING CODE*\n\n` +
                    `Code: *${pairingCode}*\n\n` +
                    `*How to use:*\n` +
