#!/bin/bash
# Kazi-md-bot-4X Fly.io One-Click Deployer
# Usage: bash <(curl -s https://raw.githubusercontent.com/kajisaheb809-cyber/Kazi-md-bot-4X/main/lib/fly.sh)
...
echo -e "${CYAN}"
echo "╔═══════════════════════════════════════╗"
echo "║        Kazi-md-bot-4X Fly.io Deployer        ║"
...
read -p "$(echo -e ${CYAN}App name (e.g. my-gaaju-md): ${NC})" APP_NAME
APP_NAME=${APP_NAME:-gaaju-md-bot}

read -p "$(echo -e ${CYAN}Session ID (GlobalTechInfo/Kazi-md-bot-4X_xxxxx): ${NC})" SESSION_ID
if [ -z "$SESSION_ID" ]; then
    echo -e "${RED}❌ Session ID is required!${NC}"
    exit 1
fi

read -p "$(echo -e ${CYAN}Owner WhatsApp number (e.g. 8801629143977): ${NC})" OWNER_NUMBER
OWNER_NUMBER=${OWNER_NUMBER:-234000000000}

read -p "$(echo -e ${CYAN}Bot name (default: Kazi-md-bot-4X): ${NC})" BOT_NAME
BOT_NAME=${BOT_NAME:-GAAJU-MD}

read -p "$(echo -e ${CYAN}MongoDB URL (recommended, press Enter to skip): ${NC})" MONGO_URL

read -p "$(echo -e ${CYAN}Timezone (default: Africa/Lagos): ${NC})" TIMEZONE
...
if [ ! -f "fly.toml" ]; then
    echo -e "${YELLOW}📦 Cloning Kazi-md-bot-4X repo...${NC}"
    git clone https://github.com/kajisaheb809-cyber/Kazi-md-bot-4X kazi-md-deploy
    cd Kazi-md-bot-4X-deploy
fi
